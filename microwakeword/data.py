...
class FeatureHandler(object):
    def __init__(
        self,
        general_negative_data_dir,
        adversarial_negative_data_dir,
        positive_data_dir,
    ):
        self.general_negative_data_index = {
            "validation": [],
            "testing": [],
            "training": [],
        }
        self.loaded_general_negative_data_features = []
        self.adversarial_negative_data_index = {
            "validation": [],
            "testing": [],
            "training": [],
        }
    ...
    def get_truncated_features(
        self, mode, type, index, features_length, truncation_strategy="random"
    ):
        if type == "background":
            candidates = self.general_negative_data_index[mode]
            sample = candidates[index]
            numpy_features = self.loaded_general_negative_data_features[
                sample["loaded_feature_index"]
            ][sample["subindex"]]
        elif type == "adversarial_negative":
            candidates = self.adversarial_negative_data_index[mode]
            sample = candidates[index]
            numpy_features = self.loaded_adversarial_negative_data_features[
                sample["loaded_feature_index"]
            ][sample["subindex"]]
        elif type == "positive":
            candidates = self.positive_data_index[mode]
            sample = candidates[index]
            numpy_features = self.loaded_positive_data_features[
                sample["loaded_feature_index"]
            ][sample["subindex"]]
    ...
    def get_random_features_for_type(
        self, mode, type, features_length, truncation_strategy
    ):
        if type == "background":
        ...
        return self.get_truncated_features(
            mode,
            type,
            sample_index,
            features_length,
    ...
    def get_data(
        ...
        batch_size,
        features_length,
        general_negative_weight=1.0,
        adversarial_negative_weight=1.0,
        positive_weight=1.0,
        general_negative_probability=0.3,
        ...
        if mode == "training":
            ...
            sample_count = (
                general_negative_count + positive_count + adversarial_negative_count
            )
        ...
        if mode == "training":
            ...
            for i in range(0, sample_count):
                if random_prob[i] < general_negative_probability:
                    data[i] = self.get_random_features_for_type(
                        mode,
                        "background",
                        features_length,
                        truncation_strategy=truncation_strategy,
                    ...
                    general_negative_probability + positive_probability
                ):
                    data[i] = self.get_random_features_for_type(
                        mode,
                        "positive",
            ...
            for index in range(len(general_negative_candidates)):
                data[index] = self.get_truncated_features(
                    mode,
                    "background",
                    index,
                    features_length,
            ...
            for index in range(len(adversarial_negative_candidates)):
                ...
                data[offset_index] = self.get_truncated_features(
                    mode,
                    "adversarial_negative",
                    index,
                    features_length,
            ...
            for index in range(len(positive_candidates)):
                offset_index = (
                    index
                    + len(general_negative_candidates)
                    + len(adversarial_negative_candidates)
                )
