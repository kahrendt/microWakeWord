...
def spectrogram_slices_dropped(flags):
    ...
    for kernel_size, dilation in zip(
        parse(flags.ds_kernel_size), parse(flags.ds_dilation)
    ):
...
def resnet_block(
    ...
    if residual:
        ...
        net_res_drop_layer = strided_drop.StridedDrop(net_res.shape[1] - net.shape[1])
        net_res = net_res_drop_layer(net_res)
