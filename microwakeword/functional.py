...
class Functional(training_lib.Model):
    ...
    def _init_graph_network(self, inputs, outputs):
        ...
        if not nest.is_nested(self._nested_inputs):
            ...
            nest.is_nested(t) for t in self._nested_inputs
        ):
            self._enable_dict_to_input_mapping = True
        elif isinstance(self._nested_inputs, dict) and not any(
            nest.is_nested(t) for t in self._nested_inputs.values()
        ):
            self._enable_dict_to_input_mapping = True
        else:
    ...
    def compute_output_shape(self, input_shape):
        ...
        if len(nest.flatten(input_shape)) != len(nest.flatten(self._input_layers)):
            raise ValueError(
                "Invalid input_shape argument "
                + str(input_shape)
                + ": model has "
                + str(len(self._input_layers))
                + " tensor inputs."
            )
        ...
        if len(depth_keys) > 1:
            for depth in depth_keys:
                ...
                for node in nodes:
                    ...
                    layer_input_shapes = nest.pack_sequence_as(
    ...
    def from_config(cls, config, custom_objects=None):
        ...
        with generic_utils.SharedObjectLoadingScope():
            ...
            model = cls(
                inputs=input_tensors, outputs=output_tensors, name=config.get("name")
            )
    ...
    def _validate_graph_inputs_and_outputs(self):
        ...
        for x in self.inputs:
            ...
            if not hasattr(x, "_keras_history"):
                ...
                raise ValueError(
                    ...
                    "Received: " + str(x) + " (missing previous layer metadata)."
                )
            ...
            if len(layer._inbound_nodes) > 1 or (
                ...
                cls_name = self.__class__.__name__
                logging.warning(
                    cls_name + " model inputs must come from "
        ...
        for batch_size in input_batch_sizes:
            if batch_size is not None:
                if (
                    consistent_batch_size is not None
                    and batch_size != consistent_batch_size
                ):
                    raise ValueError(
                        "The specified batch sizes of the Input Layers"
        ...
        for x in self.outputs:
            if not hasattr(x, "_keras_history"):
                cls_name = self.__class__.__name__
                raise ValueError(
                    "Output tensors of a " + cls_name + " model must be "
...
def _build_map_helper(
    tensor, finished_nodes, nodes_in_progress, nodes_in_decreasing_depth, layer_indices
):
...
def _should_skip_first_node(layer):
    ...
    if layer._self_tracked_trackables:
        return (
            ...
            isinstance(layer._self_tracked_trackables[0], input_layer_module.InputLayer)
        )
    else:
...
def reconstruct_from_config(config, custom_objects=None, created_layers=None):
    ...
    def process_node(layer, node_data):
        ...
        if input_tensors is not None:
            ...
            output_index = nest.flatten(output_tensors)[
                0
...
def get_network_config(network, serialize_layer_fn=None):
    ...
    Returns:
      Config dictionary.
    """
    serialize_layer_fn = serialize_layer_fn or generic_utils.serialize_keras_object
    config = {
        "name": network.name,
    }
    node_conversion_map = {}
...
class ModuleWrapper(base_layer.Layer):
    ...
    def __init__(self, module, method_name=None, **kwargs):
        ...
        self._expects_training_arg = (
            "training" in method_arg_spec.args or method_arg_spec.varkw is not None
        )
