...
def model(flags):
  ...
  for stride, filters1, filters2, kernel_size in zip(
      parse(flags.cnn2_strides), parse(flags.cnn2_filters1),
      parse(flags.cnn2_filters2), parse(flags.cnn2_kernel_sizes)):
    ...
    branch2 = conv2d_bn_delay(
        branch2, filters1, (kernel_size, 1), padding='causal', scale=flags.bn_scale, delay_val=time_buffer_size//2, use_one_step=flags.cnn2_use_one_step_stream)

    branch3 = conv2d_bn(net, filters1, (1, 1), scale=flags.bn_scale)
    branch3 = conv2d_bn_delay(
        branch3, filters1, (kernel_size, 1), padding='causal', scale=flags.bn_scale, delay_val=time_buffer_size//2, use_one_step=flags.cnn2_use_one_step_stream)
    branch3 = conv2d_bn_delay(
        branch3, filters1, (kernel_size, 1), padding='causal', scale=flags.bn_scale, delay_val=time_buffer_size//2, use_one_step=flags.cnn2_use_one_step_stream)
