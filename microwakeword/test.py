...
def streaming_model_false_accept_rate(flags, folder, tflite_model_name, features_fname):
  interpreter = tf.lite.Interpreter(model_path=os.path.join(flags.train_dir, folder, tflite_model_name))
  ...
  while end <= features_data.shape[0]:    
    ...
    if not start % 50000:
      logging.info('DipCo false accepts = %d; false accepts per hour = %f',
                   *(false_accept_count, false_accept_count/(start*0.02/3600)))

  logging.info('DipCo Total False Accepts = %d; total false accepts per hour = %f',
               *(false_accept_count, false_accept_count/(features_data.shape[0]*0.02/3600.0)))
...
def tflite_model_accuracy(
  ...
  for i in range(0, len(test_fingerprints)):
    ...
    for feature_index in range(0,sample_fingerprint.shape[0], input_feature_slices):
      new_data_to_input = sample_fingerprint[feature_index : feature_index + input_feature_slices, :]
      
      if is_quantized_model:
          new_data_to_input = quantize_input_data(new_data_to_input, input_details[0])
            
      # Input new data and invoke the interpreter
      interpreter.set_tensor(input_details[0]['index'], np.reshape(new_data_to_input, input_details[0]['shape']))
      interpreter.invoke()   
  ...
  logging.info('true positives %d, true negatives %d, false positives %d, false negatives %d',
               *(true_positives, true_negatives, false_positives, false_negatives))
  logging.info('Final tflite model test: accuracy = %f%%; recall = %f%%; precision = %f%%; fpr = %f%%; fnr = %f%%; (N=%d)',
               *(accuracy*100, recall*100, precision*100, fpr*100, fnr*100, len(test_fingerprints)))
