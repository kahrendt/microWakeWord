...
def validate_streaming(flags, config):
    ...
    (
        ...
        config, folder_name, file_name, data_set="validation"
    )
...
def train(flags, config):
    ...
    training_steps_list = config["training_steps"]
    learning_rates_list = config["learning_rates"]
    ...
    for training_step in range(1, training_steps_max + 1):
        ...
        if (training_step % config["eval_step_interval"]) == 0 or is_last_step:
            ...
            validation_metrics = validate_streaming(flags, config)
            ...
            with validation_writer.as_default():
                ...
                tf.summary.scalar(
                    "accuracy", validation_metrics["accuracy"], step=training_step
                )
                tf.summary.scalar(
                    "recall", validation_metrics["recall"], step=training_step
                )
                tf.summary.scalar(
                    "precision", validation_metrics["precision"], step=training_step
                )
                tf.summary.scalar(
                    "fpr", validation_metrics["false_positive_rate"], step=training_step
                )
                tf.summary.scalar(
                    "fnr", validation_metrics["false_negative_rate"], step=training_step
                )
                tf.summary.scalar(
                    "faph",
                    validation_metrics["false_accepts_per_hour"],
            ...
            model.save_weights(
                os.path.join(
                    config["train_dir"],
                    "train/",
                    str(int(best_false_accept_per_hour * 10000))
                    + "weights_"
                    + str(training_step),
                )
            )
            ...
            if (false_accepts_rate < best_false_accept_per_hour) or (
                false_accepts_rate < config["target_fpr"]
            ):
                if false_accepts_rate < config["target_fpr"]:
                    if validation_metrics["false_negative_rate"] < best_fnr:
                        ...
                        model.save_weights(
                            os.path.join(config["train_dir"], "best_weights")
                        )
