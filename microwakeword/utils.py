...
def convert_to_inference_model(model, input_tensors, mode):
  ...
  with tf.name_scope(scope_name):
    ...
    new_model = tf.keras.models.clone_model(model, input_tensors) # _clone_model(model, input_tensors)
...
def convert_saved_model_to_tflite(flags, 
                                  path_to_model, 
                                  folder, 
                                  fname, 
                                  quantize=False):
  ...
  def representative_dataset_gen():
    ...
    while positive_sample_count < 10 or negative_sample_count < 10:
      spectrogram = validation_fingerprints[counter]
      if validation_ground_truth[counter]:
      ...
      spectrogram[0,0] = 26.0 # guarantee one pixel is the preprocessor max
